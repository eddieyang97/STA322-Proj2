...
d: paper clip on leg - where the lower end of the paper clip aligns with the bottom edge of the leg
...
```{r}
#Read data file with measurements entered by hand
heliData = read.csv("HeliData.csv", header = TRUE)
#Convert treatments to factors
heliData$treatment = as.factor(heliData$treatment)
...
boxplot(heliData$time ~ heliData$treatment, xlab = "Treatment", ylab = "Fly Time")
par(mfrow=c(2,2))   
...
#Residual Plot
boxplot(regHeli$residual~heliData$treatment, xlab = "Treatment", ylab = "Residuals")
#Diagnostic Plots
plot(regHeli)
